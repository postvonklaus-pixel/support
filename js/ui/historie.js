...
import { formatCurrency, formatDate, escapeHtml, isToday, toast, openModal, closeModal, confirmDialog } from '../utils.js';
...
export function render() {
  ...
  document.getElementById('today-summary-value').textContent = formatCurrency(todayTotal, state.settings.currency);
  ...
  state.sales.forEach((sale) => {
    ...
    row.innerHTML = `
      ...
      <div class="sale-item-total">${formatCurrency(sale.total, state.settings.currency)}</div>
    `;
...
function openSaleDetail(saleId) {
  ...
  const itemsHtml = sale.items.map((it) => `
    <div class="cart-item">
      <div class="cart-item-info">
        ...
        <div class="cart-item-price">${it.quantity} × ${formatCurrency(it.price, state.settings.currency)}</div>
      </div>
      <strong>${formatCurrency(it.price * it.quantity, state.settings.currency)}</strong>
    </div>
  `).join('');
  let extra = '';
  if (sale.paymentMethod === 'cash') {
    extra = `
      <div class="change-row"><span>Gegeben</span><strong>${formatCurrency(sale.amountGiven, state.settings.currency)}</strong></div>
      <div class="change-row"><span>Rückgeld</span><strong>${formatCurrency(sale.change, state.settings.currency)}</strong></div>
    `;
  }
  ...
  document.getElementById('sale-detail-content').innerHTML = `
    ...
    ${itemsHtml}
    <div class="checkout-total"><span>Gesamt</span><strong>${formatCurrency(sale.total, state.settings.currency)}</strong></div>
    ${extra}
    ${movementsHtml}
  `;
